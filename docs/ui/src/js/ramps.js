...
function themeRampKeyColors(colorKeys, dest, scaleType) {
  ...
  let sortedColorKeys =
    scaleType === "diverging"
      ? colorKeys
      : orderColorsByLuminosity(colorKeys, "toLight");
  ...
  sortedColorKeys.map((key, index) => {
    ...
    let leftPosition = `calc(${Math.round(left)}% - ${Math.round(
      dotOffset,
    )}px)`;
...
function updateRamps(color, id, scaleType = "theme") {
  ...
  if (scaleType === "theme")
    chartsModeSelect = document.getElementById("chartsMode");
  ...
  if (scaleType === "diverging") {
    createPaletteInterpolationCharts(
      [color.startScale.colorsReversed, color.endScale.colors],
      chartsMode,
      scaleType,
    );
  } else {
