...
function create3dModel(dest, colorClasses, mode, scaleType = "theme") {
  ...
  if (scaleType === "theme") {
    ...
    for (let i = 0; i < images.length; i++) {
      if (!images[i].classList.contains("is-hidden"))
  ...
  const downloadFileName =
    scaleType === "theme"
      ? `${getThemeName()}_3dModel`
      : `${scaleType}Scale_3dModel`;
  const windowWidth = window.innerWidth;
  const windowHeight = window.innerHeight;
  const height =
    dest === "paletteModelWrapper" ? windowHeight - 120 : windowHeight - 260;
  let width = windowWidth - (424 + 304 + 32); // Fit to window, minus panel and padding
  if (
    dest === "sequentialModelWrapper" ||
    dest === "tabModelContent" ||
    dest === "divergingModelWrapper"
  ) {
...
function createColorData(colorClasses, mode, scaleType) {
  ...
  for (let i = 0; i < colorClasses.length; i++) {
    let currentColor =
      !scaleType || scaleType === "theme"
        ? colorClasses[i].backgroundColorScale
        : colorClasses[i].colors;
