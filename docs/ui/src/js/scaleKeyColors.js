...
function addScaleKeyColorInput(
  c,
  thisId = this.id,
  scaleType,
  index,
  ...
  let currentKeys =
    scaleType === "sequential"
      ...
      : scalePosition === "start"
        ? currentColor.startKeys
        : scalePosition === "end"
          ? currentColor.endKeys
          : currentColor.middleKey;
  let parent = thisId
    .replace("_addKeyColor", "")
    .replace("_addStartKeyColor", "")
    .replace("_addEndKeyColor", "");
  ...
  let isDisabledClass;
  if (scaleType === "sequential")
    isDisabledClass = currentKeys.length > 2 ? "" : "is-disabled";
...
function replaceScaleKeyInputsFromClass(id, scaleType, index, scalePosition) {
  ...
  if (scaleType === "diverging") {
    ...
    colorKeys =
      scalePosition === "start" ? currentColor.startKeys : currentColor.endKeys;
  }
...
function addScaleKeyColor(scaleType, e, position) {
  ...
  let currentColor =
    scaleType === "sequential" ? _sequentialScale : _divergingScale; // TODO: replace with _diverging when available
  let currentKeys =
    scaleType === "sequential"
      ? currentColor.colorKeys
      : position === "start"
  ...
  let lastColor =
    lastIndex > 0
      ? chroma(currentKeys[lastIndex - 1]).hsluv()
      : chroma(currentKeys[0]).hsluv();
  ...
  let c =
    lastColorLightness >= 50
      ? chroma.hsluv(lastColor[0], lastColor[1], fCshadeHalf)
      : chroma.hsluv(lastColor[0], lastColor[1], fCtintHalf);
  c = c.hex();
  currentKeys.push(c);
