...
function addKeyColorInput(c, thisId = this.id, currentColorName, index) {
  ...
  const currentColorIndex = _theme.colors
    .map((e) => e.name)
    .indexOf(currentColorName);
...
function addKeyColor(e) {
  ...
  let lastColor =
    lastIndex > 0
      ? d3.hsluv(currentColor.colorKeys[lastIndex - 1])
      : d3.hsluv(currentColor.colorKeys[0]);
  ...
  let c =
    lastColorLightness >= 50
      ? d3.hsluv(lastColor.l, lastColor.u, fCshadeHalf)
      : d3.hsluv(lastColor.l, lastColor.u, fCtintHalf);
