...
function createSVGuiKit() {
  ...
  textTitle.setAttribute(
    "font-family",
    "Adobe Clean, AdobeClean-Regular, Adobe Clean",
  );
  ...
  for (let i = 0; i < colors.length; i++) {
    if (!colors[i].name) {
      ...
      for (let j = 0; j < values.length; j++) {
        ...
        let textColor =
          values[j].contrast > 4.5 ? textColorInverse : textColorPositive;
...
document
  .getElementById("downloadThemeColorsSvg")
