  ...
  if (e !== undefined) {
    ...
    if (chroma.valid(value)) {
      ...
      for (let i = 0; i < colorSpaces.length; i++) {
        ...
        if (colorSpaces[i] === "RGB")
          colorString = `rgb(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "HSL")
          colorString = `hsl(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}%, ${round(colorChannels[2], 2)}%)`;
        if (colorSpaces[i] === "HSV")
          colorString = `hsv(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "LAB")
          colorString = `lab(${round(colorChannels[0], 2)}%, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "LCH")
          colorString = `lch(${round(colorChannels[0], 2)}%, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "OKLAB")
          colorString = `oklab(${round(colorChannels[0], 2)}%, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "OKLCH")
          colorString = `oklch(${round(colorChannels[0], 2)}%, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "HSLuv")
          colorString = `hsluv(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "CAM02")
          colorString = `jab(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "CAM02p")
          colorString = `jch(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "CMYK")
          colorString = `cmyk(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)}, ${round(colorChannels[3], 2)})`;
        if (colorSpaces[i] === "XYZ")
          colorString = `xyz(${round(colorChannels[0], 2)}, ${round(
            colorChannels[1],
            2,
          )}, ${round(colorChannels[2], 2)})`;
        if (colorSpaces[i] === "Pantone")
          colorString = `Pantone ${colorChannels}`;
...
document
  .getElementById("convertColorOneInput")
  .addEventListener("input", convertColor);
document
  .getElementById("convertColorOne_picker")
