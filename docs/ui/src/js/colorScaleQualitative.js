...
backgroundInput.addEventListener(
  "input",
  throttle(updateColorsIfContrast, 100),
);
...
button.addEventListener("click", function () {
  ...
  Promise.all([testColors, getKeepers]).then((resolve) => {
    ...
    const getSafeColorSet = Promise.resolve(
      getLargestSetCvdColors(testColors, _qualitativeScale.keeperColors),
    );
    getSafeColorSet.then((newSafeColors) => {
      if (valid) {
        if (newSafeColors === undefined)
          newSafeColors = getLargestSetCvdColors(testColors, keepers);
...
function testCvd(color1, color2, log) {
  ...
  for (let i = 0; i < modes.length; i++) {
    ...
    if (deltaE < minimumThreshold) {
      if (log)
        console.error(
          `${color1} conflicts with ${color2} in ${modes[i]}\nDeltaE: ${deltaE}`,
        );
      result = false;
      break;
    } else continue;
...
function getCvdSafeColors(colors, sample, random = false) {
  ...
  if (ratios)
    set = eliminateLowContrastFromSet(set, background, contrastMinimum);
      ...
      if (dest === "cvdSafeColors") {
        ...
        button.addEventListener("click", (e) => {
          if (_qualitativeScale.keeperColors.indexOf(color) < 0) {
            ...
            document
              .getElementById("qualitative_selectedColors")
              .classList.remove("isEmpty");
            createOutput();
          }
        });
      ...
      if (dest === "qualitative_selectedColors") {
        ...
        button.addEventListener("click", () => {
          ...
          if (_qualitativeScale.keeperColors.length < 1) {
            const selectedColors = document.getElementById(
              "qualitative_selectedColors",
            );
            selectedColors.classList.add("isEmpty");
            selectedColors.innerHTML =
              "Select generated colors to begin building your scale";
          }
        });
        swatch.appendChild(button);
      }
  ...
  Promise.all([getAllModes, arr]).then((resolve) => {
    ...
    modes.forEach((mode, index) => {
        ...
        simArray.then((simColor) => {
          ...
          if (sortBySimmilarity) {
            ...
            if (mode === "achromatopsia")
              colorsLch.sort((a, b) => (a.lightness > b.lightness ? 1 : -1));
      ...
      // is not an array, we need to output something other than simulated colors.
        ...
        textSpan.className = "spectrum-Body spectrum-Body--sizeXXS";
        textSpan.innerHTML =
          "No colors to preview. Cycle through colors again.";
        parentWrap.appendChild(textSpan);
      }
    });
  });
