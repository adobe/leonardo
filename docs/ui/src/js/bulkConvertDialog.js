...
function bulkItemConvertColorInput(e) {
  ...
  for (let i = 0; i < checkboxes.length; i++) {
    if (checkboxes[i].checked)
      opts.push(checkboxes[i].id.replace("checkboxConvert-", ""));
  }
...
function createColorJson(value, colorSpaces, background) {
  ...
  // If it's a valid color input, do this stuff...
    ...
    for (let i = 0; i < colorSpaces.length; i++) {
      ...
      if (colorSpaces[i] === "wcag2")
        colorChannels = round(
          contrast(color.rgb(), chroma(background).rgb(), "wcag2"),
          2,
        );
      if (colorSpaces[i] === "wcag3") {
        ...
        colorChannels = round(
          baseV < 0.5
            ? APCAcontrast(sRGBtoY(color.rgb()), sRGBtoY(base)) * -1
            : APCAcontrast(sRGBtoY(color.rgb()), sRGBtoY(base)),
          2,
        );
      }
      ...
      if (colorSpaces[i] === "RGB")
        colorChannelLabels = ["rgb (R)", "rgb (G)", "rgb (B)"];
      if (colorSpaces[i] === "HSL")
        colorChannelLabels = ["hsl (H)", "hsl (S)", "hsl (L)"];
      if (colorSpaces[i] === "HSV")
        colorChannelLabels = ["hsv (H)", "hsv (S)", "hsv (V)"];
      if (colorSpaces[i] === "LAB")
        colorChannelLabels = ["LAB (L)", "LAB (A)", "LAB (B)"];
      if (colorSpaces[i] === "LCH")
        colorChannelLabels = ["LCh (L)", "LCh (C)", "LCh (H)"];
      if (colorSpaces[i] === "OKLAB")
        colorChannelLabels = ["OKLAB (L)", "OKLAB (A)", "OKLAB (B)"];
      if (colorSpaces[i] === "OKLCH")
        colorChannelLabels = ["OKLCH (L)", "OKLCh (C)", "OKLCh (H)"];
      if (colorSpaces[i] === "HSLuv")
        colorChannelLabels = ["HSLuv (L)", "HSLuv (U)", "HSLuv (V)"];
      if (colorSpaces[i] === "CAM02")
        colorChannelLabels = ["CAM02 (L)", "CAM02 (A)", "CAM02 (B)"];
      if (colorSpaces[i] === "CAM02p")
        colorChannelLabels = ["CAM02 (L)", "CAM02 (C)", "CAM02 (H)"];
      if (colorSpaces[i] === "CMYK")
        colorChannelLabels = ["cmyk (C)", "cmyk (M)", "cmyk (Y)", "cmyk (K)"];
      if (colorSpaces[i] === "XYZ")
        colorChannelLabels = ["xyz (X)", "xyz (Y)", "xyz (Z)"];
      if (colorSpaces[i] === "Pantone") colorChannelLabels = ["Pantone"];

      if (colorSpaces[i] === "wcag2")
        colorChannelLabels = [`Rel Lum vs ${background}`];
      if (colorSpaces[i] === "wcag3")
        colorChannelLabels = [`APCA vs ${background}`];
