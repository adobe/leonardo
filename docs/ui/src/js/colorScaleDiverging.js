...
function colorScaleDiverging(scaleType = "diverging") {
  ...
  if (mq.matches) {
    ...
    document
      .getElementById("main_sequential")
      .classList.add("spectrum--darkest");
  } else {
    //light mode
  ...
  let downloadGradient = document.getElementById(
    `${scaleType}_downloadGradient`,
  );
  ...
  Promise.all([hasStartKeys, hasMiddleKey, hasEndKeys]).then(
    (divergingKeys) => {
  ...
  createPaletteInterpolationCharts(
    [colorClass.startScale.colorsReversed, colorClass.endScale.colors],
  ...
  const hasStartButton = Promise.resolve(
    document.getElementById(buttonStartId),
  );
