...
function createOutputColors(dest) {
  ...
  let themeBackgroundColorArray = [
    d3.rgb(themeBackgroundColor).r,
    d3.rgb(themeBackgroundColor).g,
    d3.rgb(themeBackgroundColor).b,
  ];
  ...
  destinations.map((dest) => {
    ...
    for (let i = dest === swatchesOutputs ? 1 : 0; i < length; i++) {
      ...
      if (theme[i].values) {
        ...
        p.className =
          "spectrum-Heading spectrum-Heading--sizeXXS themeOutputItem--Heading";
        ...
        for (let j = 0; j < theme[i].values.length; j++) {
          ...
          if (dest === swatchesOutputs)
            colorValue = colorToGrayScale(colorForTransform);

          // get the ratio to print inside the swatch
          let contrast = theme[i].values[j].contrast;
          let colorArray = [
            d3.rgb(colorForTransform).r,
            d3.rgb(colorForTransform).g,
            d3.rgb(colorForTransform).b,
          ];
          let actualContrast = Leo.contrast(
            colorArray,
            themeBackgroundColorArray,
            undefined,
            wcagFormula,
          );

          let innerTextColor =
            d3.hsluv(colorForTransform).v > 50 ? "#000000" : "#ffffff";
          let contrastRounded = Math.round(actualContrast * 100) / 100;
          let contrastTextNode =
            wcagFormula === "wcag2" ? contrastRounded + " :1" : contrastRounded;
